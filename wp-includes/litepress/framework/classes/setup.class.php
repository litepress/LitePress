...
if ( ! class_exists( Framework::class ) ) {
  class Framework {
    ...
    public function __construct() {
      ...
      add_action( 'after_setup_theme', array( Framework::class, 'setup' ) );
      add_action( 'init', array( Framework::class, 'setup' ) );
      add_action( 'switch_theme', array( Framework::class, 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( Framework::class, 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( Framework::class, 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( Framework::class, 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( Framework::class, 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( CSF_Options::class ) && ! empty( self::$args['admin_options'] ) ) {
      ...
      $params = array();
      if ( class_exists( CSF_Customize_Options::class ) && ! empty( self::$args['customize_options'] ) ) {
      ...
      $params = array();
      if ( class_exists( CSF_Metabox::class ) && ! empty( self::$args['metabox_options'] ) ) {
      ...
      $params = array();
      if ( class_exists( CSF_Nav_Menu_Options::class ) && ! empty( self::$args['nav_menu_options'] ) ) {
      ...
      $params = array();
      if ( class_exists( CSF_Profile_Options::class ) && ! empty( self::$args['profile_options'] ) ) {
      ...
      $params = array();
      if ( class_exists( CSF_Taxonomy_Options::class ) && ! empty( self::$args['taxonomy_options'] ) ) {
      ...
      $params = array();
      if ( class_exists( CSF_Comment_Metabox::class ) && ! empty( self::$args['comment_options'] ) ) {
      ...
      $params = array();
      if ( class_exists( CSF_Shortcoder::class ) && ! empty( self::$args['shortcode_options'] ) ) {
        ...
        if ( class_exists( CSF_Shortcoder::class ) && ! empty( self::$shortcode_instances ) ) {
    ...
    public static function constants() {
      ...
      $framework_dir  = esc_url( home_url( '/' ) ) . 'wp-includes/litepress/framework';
      
      self::$dir = $dirname;
      self::$url = $framework_dir;
    ...
    public static function includes() {
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( NS . 'CSF_Field_'. $field ) && class_exists( CSF_Fields::class ) ) {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = NS . 'CSF_Field_' . $field['type'];
    ...
    public static function add_typography_enqueue_styles() {
      ...
      if ( ! empty( self::$webfonts ) ) {
        ...
        if ( ! empty( self::$webfonts['enqueue'] ) ) {
          ...
          wp_enqueue_style( 'csf-google-web-fonts', esc_url( add_query_arg( $query, '//googlefonts.wp-china-yes.net/css' ) ), array(), null );
        ...
        if ( ! empty( self::$webfonts['async'] ) ) {
          ...
          wp_enqueue_script( 'csf-google-web-fonts', esc_url( '//googleajax.wp-china-yes.net/ajax/libs/webfont/1.6.26/webfont.js' ), array(), null );
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field_type ) ) {
        ...
        $classname = NS . 'CSF_Field_'. $field_type;
