...
function lp_filter_http_request( $result, $args, $url ) {
		...
		// 不是版本更新的查询请求，直接返回
		if ( false === strpos( $url, '//api.w.org.ibadboy.net/core/version-check/' ) && false === strpos( $url, '//api.litepress.cn/core/version-check/' ) && false === strpos( $url, '//api.wordpress.org/core/version-check/' ) ) {
			return $result;
		}
        ...
        $url = str_replace('//api.litepress.cn', '//beta.litepress.cn', $url);
